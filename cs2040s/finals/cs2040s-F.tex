...
\begin{itemize}
    \item 1D-array: $O(\log n)$
    \item 2D-array: $O(n \log m)$ or $O(n+m)$
    \begin{itemize}
        \item $T(n,m) = T(n/2, m/2)	+ O(n +	m)$
    \end{itemize}
\end{itemize}
...
\begin{tightcenter}
    ...
    \\* \textbf{insertion} - max 2 rotations; \textbf{deletion} - recurse up to root; 
\end{tightcenter}
...
Note: need to update nodes (weight/max) aft. rotation
...
\begin{itemize}
    ...
    \end{enumerate}
    \includegraphics[width=0.3\linewidth]{cs2040s-abtree-split-1.png}
    $\Then$
    \includegraphics[width=0.24\linewidth]{cs2040s-abtree-split-2.png}
...
\begin{itemize}
    \item increment index until found empty slot.
...
\section{SET ADT}
\begin{itemize}
    \item $\checkmark$ speed $\checkmark$ space $\checkmark$ no false negatives 
    \item $\quad\quad \times$ no ordering 
...
\begin{itemize}
    ...
    \item \code{delete} operation: store \textbf{counter} instead of 1 bit
...
\textbf{KnuthShuffle}:  $O(n)$ - for (i = n-1..0) \{ swap(i, rand(0, i)) \}

\section{AMORTIZED ANALYSIS}
\begin{tightcenter}
    an operation has \textbf{amortized cost} $T(n)$ if 
    for every integer $k$, the cost of ANY $k$ operations is $\leq kT(n)$.
\end{tightcenter}
\begin{itemize}
    ...
    \end{itemize}
    \item Amortized runtime $\leq$ worst-case.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Strongly connected}: every $v$ is reachable from $u$.
...
\begin{itemize}
    ...
    \end{itemize}
\end{itemize}
With adjacency matrix: $O(V)$ per node $\then$ total $O(V^2)$
...
\begin{itemize}
    \item any 2 subtrees of the MSTs are also MSTs (cutting an MST)
    \item for every \textbf{cycle}, the \textbf{max weight edge} is \textbf{NOT} in the MST.
    \item for every \textbf{partition} of the nodes, the \textbf{minimum weight edge across the cut} is in the MST.
    \begin{itemize}
        \item for every \textbf{vertex}, the \textbf{minimum outgoing edge} is in the MST.
    \end{itemize}
