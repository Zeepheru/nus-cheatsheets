 ...
 \begin{center}
    \fbox{%
        \parbox{0.8\linewidth}{\centering \textcolor{black}{
            {\Large\textbf{GEA1000}}
            \\ \normalsize{AY23/24 Sem 2 Finals}}
            \\ {\footnotesize Original by: \textcolor{myblue}{github.com/gerteck}}
...
\begin{itemize}
 ...
 and Spread.} \\
	\textbf{Shape}: left/right-skewed - left tails to the left, vv\\
	\textbf{Centre}: Described by Median. Cross represents mean. \\
	\textbf{Spread}: IQR - spread for the middle 50\% of the data set
\end{itemize}
...
\subsection{Bivariate EDA}
	Focus on r'ship between two variables in a population.
\begin{itemize}
...
\subsection{Correlation Coefficient, r}
	Correlation coefficient between two numerical values, r, is a \textbf{measure of linear association} between them. 
\begin{itemize}
	\item \textbf{Sign and Magnitude of r}: Tells us about the direction of the linear association. If $r>0$, association is positive, when one increases the other tends to increase as well. v.v. \\
 If $r=1$ or $r=-1$, there is perfect positive/negative association. \\
 % When $r=0$, there is no linear association. 
 % Magnitude of r tells us the strength of the linear association. 
 $|r|$: (0 - 0.3 weak, 0.3 - 0.7 moderate, 0.7 - 1 strong)
...
Computed using aggregates rather than indiv data. \\
\begin{itemize}
    \item \textbf{Ecological Fallacy} - Using ecological corr to conclude indiv level corr
    \item \textbf{Atomistic Fallacy} - Using indiv level corr to conclude ecological level corr
\end{itemize}
...
\begin{itemize}
	...
	\item Don't \textbf{extrapolate}. 
	\item \textbf{Non-Linear Data}: Model r'ship indirectly (e.g. property of log) to form linear.
\end{itemize}
...
\subsection{Conditional Probability and Independence}
	\textbf{Conditional Probability} is written using the notation $P( E \vert F)$ and read as "probability of $E$ given $F$". 
\\ ~\\	\centerline{ $P( E \vert F ) = \frac{P( E \cap F )}{ P(F)} $ } ~\\

\textbf{Bayes' Theorem} (NIS)
\begin{equation*}
...
\begin{itemize}
	...
	\centerline {$ P(A) \cdot P(B) = P(A \cap B) $}
...
% A class of continuous random variables. $N(x,y)$. 
% \begin{itemize}
...
\begin{itemize}
	...
	\textbf{Idea of confidence level}: If many random samples (of the same size) are taken, $~95\%$ of confidence intervals will contain the population parameter!
...
One-sample t-test and Chi-squared test:
    ...
    \textbf{Note for association:} \\ null - no assoc; alternative - $\exists$ assoc.
